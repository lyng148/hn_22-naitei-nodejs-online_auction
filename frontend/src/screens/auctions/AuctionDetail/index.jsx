...
const AuctionDetail = () => {
  ...
  const { days, hours, minutes, seconds, isOver } = useCountdown(auction?.endTime);
  ...
  const canBid =
    user && user.role === "BIDDER" && !isSellerOwner && !isSeller && canBidStatus && !isOver;

  const handlePlaceBid = async () => {

    showNotification("Sau thêm đặt Bid ở đây nhé", "error"); // An
  };
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Caption className="text-gray-700 bg-gray-100 px-3 py-1 text-sm rounded-full">
                Min step: ${formatMoney(auction.minimumBidIncrement)}
              </Caption>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow p-4 sticky top-24">
              <Title level={4} className="mb-2">Place a bid</Title>
              ...
              <PrimaryButton
                ...
                disabled={!canBid || Number(bidInput) < minBid}
              >
                {canBid ? "Place Bid" : "Bidding not available"}
              </PrimaryButton>
              {!user && (
