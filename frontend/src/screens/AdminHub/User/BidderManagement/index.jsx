...
const BidderManagement = () => {
  ...
  const fetchBidders = useCallback(async (page = 1, limit = 10) => {
    try {
      ...
      showToastNotification("Failed to fetch bidders", "error");
    } finally {
      setLoading(false);
    }
  ...
  const handlePageChange = (newPage) => {
    fetchBidders(newPage, pagination.limit);
  };
    ...
    return (
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-2 text-gray-600">Loading bidders...</span>
        </div>
      </div>
    );
  }
      ...
      {!loading && filteredUsers.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            {searchTerm ? "Try adjusting your search criteria" : "No bidders are currently registered"}
          </p>
        </div>
      )}
